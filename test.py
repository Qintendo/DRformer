...
parser.add_argument('--weights', default='./log/DRFormer_istd/models/model_best.pth',
    type=str, help='Path to weights')
parser.add_argument('--gpus', default='0', type=str, help='CUDA_VISIBLE_DEVICES')
parser.add_argument('--arch', default='DRFormer', type=str, help='arch')
...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        if args.cal_metrics:
            ...
            ssim_val_rgb.append(ssim_loss(gray_restored, gray_gt, channel_axis=None, data_range=gray_gt.max() - gray_gt.min()))
            ssim_val_ns.append(ssim_loss(gray_restored * (1 - bm.squeeze()), gray_gt * (1 - bm.squeeze()), channel_axis=None, data_range=gray_gt.max() - gray_gt.min()))
            ssim_val_s.append(ssim_loss(gray_restored * bm.squeeze(), gray_gt * bm.squeeze(), channel_axis=None, data_range=gray_gt.max() - gray_gt.min()))
