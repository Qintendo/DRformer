...
class CALayer(nn.Module):
    ...
    def forward(self, x, share_weights=None, factor=None):
        y = self.avg_pool(x)
        y = self.conv_du(y)
        if share_weights is not None and factor is not None:
            y = y * (1 - factor) + share_weights * factor
        return x * y, y
...
class CAB(nn.Module):
    ...
    def forward(self, x, share_weights=None, factor=None):
        res = self.body(x)
        res, share_weights = self.CA(res, share_weights, factor)
        res += x
        return res, share_weights
...
#########################################
########### DR Transformer #############
class DRTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=10, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
        ...
        # three paths
        self.CAB1 = CAB(dim, kernel_size=3, reduction=4, bias=False, act=nn.PReLU())
        self.CAB2 = CAB(dim, kernel_size=3, reduction=4, bias=False, act=nn.PReLU())
        self.CAB3 = CAB(dim, kernel_size=3, reduction=4, bias=False, act=nn.PReLU())
    ...
    def share_weight_factor_generator(self, s_share, ns_share, lambda_param=16, threshold=0.3):
        diff = torch.abs(s_share - ns_share)
        threshold = torch.full_like(diff, threshold)
        factor = 1 / (1 + torch.exp(lambda_param * (diff - threshold)))
        return factor
    ...
    def forward(self, x, xm, mask=None, img_size = (128, 128)):
        ...
        x = rearrange(x, ' b (h w) (c) -> b c h w ', h=H, w=W)
        xm = rearrange(xm, 'b h w c -> b c h w')

        one = torch.ones_like(xm)
        xm_s = torch.where(xm < 0.1, one, one*0.2) # shadow 1, non-shadow 0.2
        xm_ns = torch.where(xm < 0.1, one*0.2, one) # shadow 0.2, non-shaodw 1

        x_s = x
        x_ns = x

        x_s, s_share = self.CAB1(x_s)
        x_ns, ns_share = self.CAB2(x_ns)

        s_share = s_share.detach()
        ns_share = ns_share.detach()
        factor = self.share_weight_factor_generator(s_share, ns_share)
        share_weight = ns_share

        x, _ = self.CAB3(x, share_weight, factor)
...
#########################################
########### Basic layer of DRFormer ################
class BasicDRFormer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        if cab:
            ...
            self.blocks = nn.ModuleList([
                DRTransformerBlock(dim=dim, input_resolution=input_resolution,
                                     num_heads=num_heads, win_size=win_size,
                                     shift_size=0 if (i % 2 == 0) else win_size // 2,
                                     mlp_ratio=mlp_ratio,
                                     qkv_bias=qkv_bias, qk_scale=qk_scale,
...
class DRFormer(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
        ...
        # Encoder
        self.encoderlayer_0 = BasicDRFormer(dim=embed_dim,
        ...
        self.encoderlayer_1 = BasicDRFormer(dim=embed_dim*2,
        ...
        self.encoderlayer_2 = BasicDRFormer(dim=embed_dim*4,
        ...
        self.encoderlayer_3 = BasicDRFormer(dim=embed_dim*8,
        ...
        # Bottleneck
        self.conv = BasicDRFormer(dim=embed_dim*16,
        ...
        self.decoderlayer_0 = BasicDRFormer(dim=embed_dim*16,
                            output_dim=embed_dim*16,
                            input_resolution=(img_size // (2 ** 3),
                                                img_size // (2 ** 3)),
                            depth=depths[5],
        ...
        self.decoderlayer_1 = BasicDRFormer(dim=embed_dim*8,
                            output_dim=embed_dim*8,
                            input_resolution=(img_size // (2 ** 2),
                                                img_size // (2 ** 2)),
                            depth=depths[6],
        ...
        self.decoderlayer_2 = BasicDRFormer(dim=embed_dim*4,
                            output_dim=embed_dim*4,
                            input_resolution=(img_size // 2,
                                                img_size // 2),
                            depth=depths[7],
        ...
        self.decoderlayer_3 = BasicDRFormer(dim=embed_dim*2,
                            output_dim=embed_dim*2,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[8],
    ...
    def forward(self, x, xm, mask=None):
        ...
        conv1 = self.encoderlayer_1(pool0, xm0, mask=mask, img_size = self.img_size)
        pool1 = self.dowsample_1(conv1, img_size = self.img_size)
        m = nn.MaxPool2d(2)
        xm1 = m(xm0)
        self.img_size = (int(self.img_size[0] / 2), int(self.img_size[1] / 2))

        conv2 = self.encoderlayer_2(pool1, xm1, mask=mask, img_size = self.img_size)
        pool2 = self.dowsample_2(conv2, img_size = self.img_size)
        self.img_size = (int(self.img_size[0] / 2), int(self.img_size[1] / 2))
        m = nn.MaxPool2d(2)
        xm2 = m(xm1)

        conv3 = self.encoderlayer_3(pool2, xm2, mask=mask, img_size = self.img_size)
        ...
        # Bottleneck
        conv4 = self.conv(pool3, xm3, mask=mask, img_size = self.img_size)
        ...
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv2 = torch.cat([up2,conv1],-1) # (7,25600,128)
        deconv2 = self.decoderlayer_2(deconv2, xm0, mask=mask, img_size = self.img_size)

        up3 = self.upsample_3(deconv2, img_size = self.img_size)
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv3 = torch.cat([up3,conv0],-1) # (7,102400,64)
